...
1. `Twitter Sentiment Analysis using NLTK, Python for Donald Trump.ipynb` is a local test on Jupyter Notebook to train a model before implementing on SageMaker.
2. `Scikit-learn\_Estimator\_Example\_For\_RandomForestClassifier\_Classifier.ipynb` is an implementation on SageMaker with own scikit-learn container in Amazon ECR.
 
## SageMaker scikit-learn RandomForest container sample
...
* __build\_and\_push.sh__: The script to build the Docker image (using the Dockerfile above) and push it to the [Amazon EC2 Container Registry (ECR)][ecr] so that it can be deployed to SageMaker. Specify the name of the image as the argument to this script. The script will generate a full name for the repository in your account and your configured AWS region.

* __randomforest__: The directory that contains the application to run in the container. See the next session for details about each of the files.
...
The tree under test-dir is mounted into the container and mimics the directory structure that SageMaker would create for the running container during training or hosting. We use csv format test data for local testing.

* __input/config/hyperparameters.json__: The hyperparameters for the training job.
* __input/data/training/train\_tweets.csv__: The training data.
